 ...
 * @since 2023/12/18
 **/
public class TotalSRWithAnomalyDetection2 {
    /**
    ...
    public TotalSRWithAnomalyDetection2() {
    }

    public void runAlgorithm(String input, String output,
                             double minConfidence, double minutil) throws IOException {
        ...
        if (database == null) {
            try {
                ...
                database.loadFile(input, Integer.MAX_VALUE);
...
//            database.print();
        ...
        // all rules
        if (minutil > 1) this.minutil = minutil;
        if (this.minutil == 0) {
            this.minutil = 0.001;
        }
